...
For fast type checking, consider [pyrefly](https://pyrefly.org/) from Meta (successor to Pyre). Written in Rust, it can check 1.8M lines/sec — significantly faster than mypy or pyright.
...
**Key features:**
- Automatic virtual environment and conda detection
- Project discovery from `pyproject.toml` or `pyrefly.toml`
- Advanced type inference (even for unannotated code)
- VS Code extension with real-time autocomplete and navigation
...
**When to use pyrefly vs alternatives:**
- `pyrefly` - fastest, strong IDE features, good for CI and large codebases (actively evolving)
